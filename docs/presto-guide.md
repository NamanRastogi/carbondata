...
This means current version of carbon supports presto 310 version.
...
Currently carbondata supports only one version of presto, cannot handle multiple versions at same time. If user wish to use older version of presto, then need to use older version of carbon (other old branches, say branch-1.5 and check the supported presto version in it's pom.xml file in integration/presto/)
  ...
  1. Download that version of Presto (say 310) using below command:
  ```
  wget https://repo1.maven.org/maven2/io/prestosql/presto-server/310/presto-server-310.tar.gz
  ```

  2. Extract Presto tar file: `tar zxvf presto-server-310.tar.gz`.

  3. Download the Presto CLI of the same presto server version (say 310) for the coordinator and name it presto.

  ```
    wget https://repo1.maven.org/maven2/io/prestosql/presto-cli/310/presto-cli-310-executable.jar

    mv presto-cli-310-executable.jar presto
 ...
 ### Create Configuration Files
  ...
  1. Create `etc` folder in presto-server-310 directory.
...
* Download presto server (310 is suggested and supported) : https://repo1.maven.org/maven2/io/prestosql/presto-server/
...
* Download presto cli client of version 310 : https://repo1.maven.org/maven2/io/prestosql/presto-cli/
